...
- (void)setWithCoder:(NSCoder *)aDecoder
{
    ...
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                [NSException raise:ALPHASerializationException format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
            }
...
{
    //
    // Only serializable items can be serialized, this is just a fail-safe check.
    //
    if (![self conformsToProtocol:@protocol(ALPHASerializableItem)])
    {
        return;
    }
