...
@interface ALPHABonjourDataChunk ()

@property (nonatomic, assign) NSUInteger sequenceNumber;

@end

@implementation ALPHABonjourDataChunk
{
    NSMutableData *_data;
    ...
    ALPHABonjourDataConnectionContentType _encoding;
    
    NSRange _rangeOfHeader;
    NSUInteger _contentLength;
}

- (id)initWithObject:(id)object encoding:(ALPHABonjourDataConnectionContentType)encoding error:(NSError **)error
{
    self = [super init];
...
- (BOOL)_encodeObject:(id)object error:(NSError **)error
{
    ...
    switch (_encoding)
    {
        case ALPHABonjourDataConnectionContentTypeJSON:
        {
            ...
            if (![NSJSONSerialization isValidJSONObject:object])
            {
                if (error)
                {
                    ...
                    *error = [NSError errorWithDomain:ALPHABonjourDataConnectionErrorDomain code:1 userInfo:userInfo];
                }
        ...
        case ALPHABonjourDataConnectionContentTypeNSCoding:
        {
            // check if our sending encoding type permits this object
...
- (id)decodedObject
{
    ...
    if (_encoding == ALPHABonjourDataConnectionContentTypeJSON)
    {
...
- (void)_decodeHeader
{
    ...
    {
        _encoding = ALPHABonjourDataConnectionContentTypeJSON;
    }
    else if ([contentType isEqualToString:@"application/octet-stream"])
    {
        _encoding = ALPHABonjourDataConnectionContentTypeNSCoding;
    }
    else
    {
