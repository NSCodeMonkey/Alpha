...
@interface ALPHABonjourServer() <NSNetServiceDelegate, ALPHABonjourDataConnectionDelegate>
...
{
    ALPHABonjourServer *server = (__bridge ALPHABonjourServer *)info;
...
@implementation ALPHABonjourServer
{
    ...
    __weak id <ALPHABonjourServerDelegate> _delegate;
}
...
- (void)stop
{
    ...
    for (ALPHABonjourDataConnection *connection in [_connections copy])
    {
        [connection close];
    }
...
- (void)_acceptConnection:(CFSocketNativeHandle)nativeSocketHandle
{
    ALPHABonjourDataConnection *newConnection = [[ALPHABonjourDataConnection alloc] initWithNativeSocketHandle:nativeSocketHandle];
    
    //ALPHABonjourDataConnection *newConnection = [[ALPHABonjourDataConnection alloc] initWithService:service];
...
{
    for (ALPHABonjourDataConnection *connection in _connections)
    {
        NSError *error;
